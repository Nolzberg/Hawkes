...
simu_multi = function(lambda0,a,delta,beta,N){
  ...
  for(k in 1:N){
    ...
    ## Uniquement pour vérifier l'avancée de l'algorithme
...
plot_inte = function(Tn,Nk,Y,lambda0,a,delta){
  ...
  col = rainbow(m)
  ma = max(val)
  plot(time,val[1,],type='l',col = col[1], ylim = c(0,ma),main = TeX("Intensité du processus de Hawkes avec 10 neurones"), xlab = "t",ylab = TeX("$\\lambda (t)$"))
  for(p in 2:m){
...
plot_champMoy = function(temps,val){
  ...
  plot(temps,valmoy,type = 'l',col = 'purple',ylim = c(0,max(val)),main = TeX("Intensité moyenne du processus de Hawkes multivarié"), xlab = "t",ylab = TeX("$\\lambda (t)$"))
...
## ATTENTION : si vous n'avez pas une bonne machine, il ne faut pas mettre un nb et un N très grand (environ 200 / 5000)
...
plot(pest100$Temps,pest100$Intensite,type = 'l',col = 'red',xlim = c(0,lim),main = TeX("Intensité du neurone"), xlab = "t",ylab = TeX("$\\lambda (t)$"))
